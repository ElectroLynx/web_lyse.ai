...
const NOT_FOUND_PAGE = path.join(ROOT, 'subpages', '404.html');
...
const server = http.createServer((req, res) => {
  ...
  const serveFile = (pathToFile, onMissing, statusCode = 200) => {
  ...
  const serveNotFound = () => serveFile(NOT_FOUND_PAGE, undefined, 404);

  if (url !== '/index.html' && !path.extname(url)) {
    const slug = url.replace(/^\/+|\/+$/g, '').replace(/[^a-zA-Z0-9_-]/g, '_').replace(/_+/g, '_');
    if (slug) {
      return serveFile(path.join(ROOT, 'subpages', slug + '.html'), serveNotFound);
    }
    return serveNotFound();
  }

  serveFile(filePath, serveNotFound);
});
